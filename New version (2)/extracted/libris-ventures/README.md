...
Firebase Services
├── Firestore
│   ├── registry_entries   PUBLIC  — hash, certRef, timestamp, status (allow read: true)
│   ├── registry_private   LOCKED  — email, paddleTransactionId, otsProof (Cloud Functions only)
│   ├── magic_links        LOCKED  — short-lived tokens with TTL auto-delete
│   └── mail               LOCKED  — email queue (Firebase Trigger Email extension)
├── Cloud Functions
│   ├── paddleWebhook        — payment → split write to both collections
│   ├── sendMagicLink        — email → 30-min signed token
│   ├── validateMagicLink    — token → certificate list (joins private→public)
│   ├── upgradeStamps        — daily cron, OTS pending→confirmed
│   └── cleanupMagicLinks    — daily cron, delete expired tokens
└── Storage              (future: server-side PDF generation)
...
**Data separation:** Public certificate data (hash, ref, status) is readable by anyone for independent verification. Private data (email, payment details, OTS proof bytes) is stored in a separate locked collection accessible only via Cloud Functions. Certificate references are random (LV-AH-2026-XKW-MPD-RBT), not sequential, preventing registry enumeration.
...
1. **registry_private** — for magic link certificate lookup:
   - Fields: `registrantEmail` (Ascending), `timestamp` (Descending)

2. **magic_links** — for cleanup query:
   - Fields: `expiresAt` (Ascending)

#### Firestore TTL Policy

Set up automatic deletion of expired magic links. In the Firebase Console:
1. Go to Firestore → TTL Policies
2. Add policy: Collection `magic_links`, field `expiresAt`

This automatically deletes documents once `expiresAt` passes. The `cleanupMagicLinks` Cloud Function provides a belt-and-suspenders backup.
...
- **Split collections**: Public data (hash, ref, status) in `registry_entries` (readable by anyone). Private data (email, payment ID, OTS proof) in `registry_private` (Cloud Functions only). Prevents email harvesting via the public verification API
- **Random certificate references**: `LV-AH-2026-XKW-MPD-RBT` format with ~19.7 billion combinations per year. Prevents enumeration attacks on sequential IDs
...
- **TTL auto-delete**: Expired magic link tokens deleted automatically by Firestore TTL policy
...
│   └── src/index.ts        paddleWebhook, sendMagicLink, validateMagicLink, upgradeStamps, cleanupMagicLinks
